...
| Piper | Optional local TTS | Optional `piper-tts` runtime install; disabled by default | Current Piper runtime is GPL-3.0. Voice models may have separate per-voice/dataset licenses. No voice is downloaded automatically. The curated IT/EN catalog is informational and installation remains explicit. |
