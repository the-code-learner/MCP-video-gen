...
| Piper / `piper-tts` | Lightweight local TTS | Pinned Python runtime dependency `piper-tts==1.6.0`; voices remain separately installed under `/data/piper/voices` | Piper runtime is GPL-3.0-or-later. Video Gen auto-enables Piper only when the runtime is importable, unless `PIPER_ENABLED` explicitly overrides the state. Voice models may have separate per-voice/dataset licenses; no voice is downloaded automatically. |
