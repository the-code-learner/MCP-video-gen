...
MCP Video Gen itself is licensed under the Apache License 2.0. The server can install, download, or invoke independent third-party components at runtime. Those components remain subject to their own upstream licenses and are not relicensed by this repository.
...
| RNNoise | Speech denoising | Debian shared library | BSD-3-Clause upstream. |
...
The public repository does not include third-party model binaries or voice models. Where the default deployment downloads a model automatically, the URL and SHA-256 are explicit in `video-mcp.yml` / `scripts/prepare_media_tools.sh`, and the artifact is stored only in the deployment's persistent `/data` volume.
