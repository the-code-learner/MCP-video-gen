...
6. Prefer native file/resource references and server-side streaming over base64. If a complete file is already available as base64 and comfortably fits one tool call, try `cache_file_base64` once. If that one-shot call is rejected or truncated by the client/tool layer, create ONE chunked session for the unchanged source file. Prefer `file_upload_chunk_auto` over the legacy explicit-offset `file_upload_chunk`: the server owns the offset, validates the complete decoded chunk before append, and leaves progress unchanged when a chunk is rejected. Use `file_upload_status` when authoritative progress is needed. After a successful import, reuse the returned `file_id`; never re-encode or re-upload the same unchanged artifact.

7. Programmatic Tool Calling (PTC) is a CLIENT/API capability, not something this MCP server can activate itself. If the client supports PTC AND the programmatic runtime can access the source bytes, use PTC for the bounded deterministic upload stage: `file_upload_begin` -> repeated `file_upload_chunk_auto` calls until `complete_by_size=true` -> `file_upload_finish`. Do not return to the language model between successful chunks. Use only the documented result fields, stop on validation ambiguity, and do not repeat completed chunks. If PTC or source-byte access is unavailable, use the same tools directly as a fallback.

8. ComfyUI output should be normalized into the Video Gen cache with `cache_output`. Return cached files to the client with `get_cached_file_resource`. Pass cached files to Blender and HyperFrames using their documented `file_id` adapters rather than temporary public URLs.

9. Do not invent integrations or capabilities, but do not assume an external integration is absent merely because Video Gen has no dedicated MCP tool for it. ComfyUI custom nodes can expose third-party services and may require their own API keys or credentials. When uncertain, inspect `list_loaded_nodes`, `get_node_definition`, `inventory_summary`, `external_backends_status`, `advanced_capabilities`, and `build_status` before deciding whether a capability is available or configured.

10. Cache files are persistent by default. Automatic cache deletion is disabled unless the deployment explicitly enables a retention policy. Use `cache_status` to inspect the active policy, `cache_pin` to protect important artifacts, `cache_unpin` to remove protection, and `cache_cleanup(dry_run=true)` before destructive manual cleanup. Pinned files must never be removed by retention cleanup.

11. The WebGUI at `/` can be used to inspect/upload/download/delete/pin cached files and inspect/export the sanitized persistent activity log. The activity log never intentionally stores secrets, full binary/base64 payloads, or signed-URL query strings.
