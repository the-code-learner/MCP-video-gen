...
TMP = (DATA_ROOT / "tmp").resolve()
HF_ROOT = Path(
    os.getenv("HYPERFRAMES_PROJECTS_ROOT", str(DATA_ROOT / "hyperframes" / "projects"))
).resolve()
...
def validate_runtime_config() -> None:
    if CF_VERIFY and (not CF_TEAM or not CF_AUD):
        raise RuntimeError(
            "CF_ACCESS_VERIFY=true requires CF_ACCESS_TEAM_DOMAIN and CF_ACCESS_AUD"
        )


def ensure_runtime_dirs() -> None:
    """Create writable runtime directories only when the application starts."""
...
async def comfy(method: str, path: str, **kwargs: Any) -> Any:
    ...
    async with httpx.AsyncClient(
        timeout=kwargs.pop("timeout", 120), follow_redirects=True
    ) as client:
    ...
    try:
        ...
        raise RuntimeError(
            f"ComfyUI HTTP {response.status_code}: {response.text[:4000]}"
        ) from exc
...
async def list_loaded_nodes(search: str = "") -> dict[str, Any]:
    ...
    return {
        ...
        if q in k.lower() or q in str(v.get("category", "")).lower()
    }
...
async def cache_output(
    filename: str, subfolder: str = "", output_type: str = "output"
) -> dict[str, Any]:
    ...
    return file_meta(
        file_id, out, "comfyui", subfolder=subfolder, type=output_type
    )
...
async def list_cached_outputs(limit: int = 200) -> list[dict[str, Any]]:
    metas = []
    for path in sorted(
        EXPORTS.glob("*.json"), key=lambda p: p.stat().st_mtime, reverse=True
    )[:limit]:
        try:
...
async def get_output_inline_base64(file_id: str) -> dict[str, Any]:
    ...
    if path.stat().st_size > MAX_INLINE_MB * 1024 * 1024:
        raise ValueError(
            "Output exceeds MAX_INLINE_OUTPUT_MB; use download URL/path instead"
        )
    return {
        "file_id": file_id,
...
async def extract_frame(
    file_id: str, seconds: float, output_filename: str = "frame.png"
) -> dict[str, Any]:
...
async def transcode_video(
    file_id: str,
    output_filename: str = "transcoded.mp4",
    crf: int = 18,
    preset: str = "medium",
) -> dict[str, Any]:
...
async def concat_videos(
    file_ids: list[str], output_filename: str = "assembled.mp4"
) -> dict[str, Any]:
...
async def overlay_video(
    ...
    await command(
        [
            ...
            str(overlay),
            "-filter_complex",
            f"[0:v][1:v]overlay={x}:{y}:eof_action=pass[v]",
            "-map",
            "[v]",
            "-map",
            "0:a?",
...
@mcp.tool()
async def hyperframes_write_text_file(
...
@mcp.tool()
async def hyperframes_read_text_file(
    project_id: str, relative_path: str, max_chars: int = 50000
) -> str:
    project = hf_project(project_id)
    return under(project, relative_path).read_text(
        encoding="utf-8", errors="replace"
    )[:max_chars]


@mcp.tool()
async def hyperframes_import_cached_media(
    ...
    return {
        "project_id": project_id,
        "destination": str(dest.relative_to(project)),
        "source_file_id": file_id,
    }
...
async def hyperframes_render(
    project_id: str,
    output_filename: str = "hyperframes.mp4",
...
class CloudflareJWT:
    def __init__(self, app):
        self.app = app
        self.keys = (
            PyJWKClient(f"{CF_TEAM}/cdn-cgi/access/certs") if CF_VERIFY else None
        )
    ...
    async def __call__(self, scope, receive, send):
        ...
        if not token:
            return await PlainTextResponse(
                "Missing Cloudflare Access JWT", status_code=403
            )(scope, receive, send)
        try:
            key = await asyncio.to_thread(
                self.keys.get_signing_key_from_jwt, token  # type: ignore[union-attr]
            )
            ...
            return await PlainTextResponse(
                "Invalid Cloudflare Access JWT", status_code=403
            )(scope, receive, send)
        return await self.app(scope, receive, send)
